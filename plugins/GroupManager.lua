--Begin GroupManager.lua By @Arashsudo
...
user_name = check_markd

own(data.first_name_)
end
if administration[tostring(arg.chat_id)]['owners'][tostring(data.id_)] then
    if not lang then
...
administration[tostring(arg.chat_id)]['owners'][tostring(data.id_)] =
...
administration[tostring(arg.chat_id)]['whitelist'][tostring(data.id_)] = nil
  ...
  if administration[tostring(arg.chat_id)]['owners'][tostring(data.id_)] then
    ...
    return tdcli.sendMessage(arg

.chat_id, "", 0, "》👤کاربر"..user_name.." *"..data.id_.."* *از قبل مدیر بود*⚠️", 0, "md")
      end
   end
administration[tostring(arg.chat_id)]['owners'][tostring(data.id_)] = user_name
    save_data(_config.moderation.data, administration)
...
local function delmsg (TiGeR,Arashsudo)
    msgs = Arashsudo.msgs 
    for k,v in pairs(Arashsudo.messages_) do
        msgs = msgs - 1
        tdcli.deleteMessages(v.chat_id_,{[0] = v.id_}, dl_cb, cmd)
        if msgs == 1 then
            tdcli.deleteMessages(Arashsudo.messages_[0].chat_id_,{[0] = Arashsudo.messages_[0].id_}, dl_cb, cmd)
            return false
        end
    end
    tdcli.getChatHistory(Arashsudo.messages_[0].chat_id_, Arashsudo.messages_[0].id_,0 , 100, delmsg, {msgs=msgs})
end
...
if not lang then
 return "》*#lø¢κ* _ρøšт łš αlяeαđч lø¢κeđ_🔒\n*〰〰〰〰〰〰〰〰*\n*øяđeявч*: [@"..check_markdown(msg.from.username or "Arashsudo").."]"
elseif lang then
 return "》ارسال #لینک در گروه  از قبلا ممنوع است🔒\n*〰〰〰〰〰〰〰〰*\nسفارش توسط : [@"..check_markdown(msg.from.username or "Arashsudo").."]"
end
else
...
if not lang then
 return "》*#υšeяиαмe* _ρøšт łš αlяeαđч lø¢κeđ_🔒\n*〰〰〰〰〰〰〰〰*\n*øяđeявч*: [@"..check_markdown(msg.from.username or "Arashsud").."]"
elseif lang then
 return "》ارسال #یوزرنیم در گروه  از قبلا ممنوع است🔒\n*〰〰〰〰〰〰〰〰*\nسفارش توسط : [@"..check_markdown(msg.from.username or "Arashsud").."]"
end
else
...
if not lang then
 return "#føšħ _ρøšт łš αlяeαđч lø¢κeđ_🔒\n*〰〰〰〰〰〰〰〰*\n*øяđeявч*: [@"..check_markdown(msg.from.username or "Aras
...
fløøđłиg* _ρøšт łš αlяeαđч lø¢κeđ_🔒\n*〰〰〰〰〰〰〰〰*\n*øяđeявч*: [@"..check_markdown(msg.from.username or "Arashsudo").."]"
elseif lang then
...
local lock_markdown = data[tostring(target)]["settings"]["lock
...
.check_markdown(msg.from.username or "Arashsudo").."]"
elseif lang then
...
rkdown(msg.from.username or "Arashsudo").."]"
elseif lang then
...
eturn "*#мυтe lø¢αтłøи* _łš αlяeαđч eиαвleđ_🔇\n*〰〰〰〰〰〰〰〰*\n*øяđeявч*: [@"..check_markdown(msg.from.username or "Arashsudo").."]"
elseif lang then
...
if matches[1] == "setowner" and is_admin(msg) or  matches[1] ==

"تنظیم مالک" and is_admin(msg) or matches[1] == "Setowner" and is_admin(msg) then
if not matches[2] and msg.reply_id then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.to.id,
...
if matches[2] == "tgservice

"  or matches[2]=="سرویس تلگرام" then
...
function callback_link (arg, data)
   local hash = "gp_lang:"..msg.to.id
   local lang = redis:get(hash)
    local administration = load_data(_config.moderation.data) 
...
if matches[1]:lower() == 'setchar' and is_mod(msg) or matches[1]:lower() == 'Setchar'  and is_mod(msg) or matches[1]:lower() == 'حداکثر حروف مجاز' and is_mod(msg) then
  ...
  if matches[1] == 'setflood' and is_mod(msg) or matches[1] == 'حساسیت اسپم' and is_mod(msg) or matches[1] == 'Setflood' and is_mod(msg) then
   ...
   save_data(_config.moderation.data, d
...
== 'clean' and is_admin(msg) or matches[1]:lower() == 'پاک کردن' and is_admin(msg) or matches[1]:lower() == 'Clean' and is_admin(msg) then
...
if not lang then
text = [[
̶̶Ƭ̶iGeR̶ ̶M̶υ̶т̶є̶т̶ι̶м̶є̶ ̶Ƈ̶σ̶м̶м̶α̶η̶ɗ̶ѕ:
