...
    return tdcli.sendMessage(arg.chat_id, "", 0, "_User_ "..user_name.." *"..data.id_.."* _is not a_ *sudo
...
     if matches[1]:lower() == 'add' and not redis:get('ExpireDate:'..msg.to.id) then
  ...
  if string.match(matches[2], '^-%d+$') then
   ...
   if lang then
    ...
    tdcli.sendMessage(SUDO, msg.id_, 1, 'با موفقیت تورو به گروه '..matches[2]..'
...
                                file_dl(sticker_id)
        ...
        if lang then
         ...
         tdcli.sendMessage(msg.to.id, msg.id_,1, '<b>استیکر</b> <code>'..folder..'</code> <b>ذخیره شد.</b>',
...
                                file_dl(anim_id)
        ...
        if lang then
         ...
         tdcli.sendMessage(msg.to.id, msg.id_,1, '<b>

تصویر متحرک</b> <code>'..folder..'</code> <b>ذخیره شد.</b>', 1, 'html')
...
1, nil, send_file, '@Arashsudo', dl_cb, nil)
...
1, nil, plug, '@Arashsudo', dl_cb, nil)
