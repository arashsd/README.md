...
local function getRecoveryEmail(password, d
...
-- Changes current ttl setting

 in a secret chat and sends corresponding message
-- @chat_id Chat identifier
...
-- @is_personal Does result of the query can be cached onl

y for specified user
...
local function addChatMember(chat_id, user_id, forward_limit, dl_cb, cmd)
  tdcli_function ({
    ...
    chat_id_ = chat_id,
    user_id_ = user_id,
    forward_limit_ = forward_limit o
...
-- @query Query to search for, can be em

pty to return all contacts
...
local function deleteSavedAnimation(an
...
local function sendSticker(chat_id, reply_to_message_id, disable_notification, from_background, reply_markup, sticker, dl_cb, cm
